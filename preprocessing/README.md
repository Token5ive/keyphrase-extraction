...
# 전처리 실행 및 저장
results = run_pipeline(save=True, base_save_path="output")
...
# 메모리에만 로드
results = run_pipeline(save=False)
train_df = results['train']
```
...
results = run_pipeline(save=True, base_save_path="custom_output")
```
...
```python
from preprocessing.load_data import load_kp20k
...
def main():
    ...
    # 전처리 실행
    results = run_pipeline(save=True, base_save_path="data/processed")
    ...
    print("\n=== 전처리 완료 ===")
    print("저장 위치: data/processed/{train|validation|test}_clean/")
